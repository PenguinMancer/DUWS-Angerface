...
{if (((getPos _x) distance (getPos _mounted_veh))<200) then {_veh_refit_fob = true};} forEach Array_of_FOBS;

// check if near the officer
if (((getPos hq_blu1) distance (getPos _mounted_veh))<200) then {_veh_refit_fob = true};

if (!(_veh_refit_fob)) exitWith {
